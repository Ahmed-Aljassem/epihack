...
export default function ReportFlow({
  ...
  const submit = async () => {
    ...
    const resolvedSymptoms = symptoms.flatMap((symptom) => {
      if (symptom !== 'Other') return [symptom];
      const trimmedOther = otherSym.trim();
      return trimmedOther ? [trimmedOther] : [];
    });
    ...
    const payload = feeling === 'sick' ? {
      ...
      other_symptom_text: symptoms.includes('Other') ? otherSym.trim() : null,
      ...
      submitted_at: submittedAt.toISOString(), id: uid(),
    } : {
      feeling: 'good',
      category: cats,
      observations,
    ...
    const savedReport: SavedReport = feeling === 'sick' ? {
      id: payload.id,
      feeling: 'sick',
      category: cats,
      symptoms: resolvedSymptoms,
      otherSymptomText: otherSym.trim() || undefined,
      severity: severity.toLowerCase(),
      diagnosis: diagnosed === 'Yes' ? diagName : undefined,
      ...
      category: cats,
      observations,
      notes: goodNotes.trim() || undefined,
      zip,
      date: submittedAt.toLocaleDateString(),
      submittedAt: submittedAt.toISOString(),
    };
