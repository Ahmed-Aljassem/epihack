...
import {
  View, Text, ScrollView, TouchableOpacity, RefreshControl, Alert, Animated, Modal, Linking,
} from 'react-native';
...
import { getMyReports, getReportStats, getUserZip, type SavedReport } from '@/utils/storage';
import { useLang } from '@/utils/i18n';
import ReportFlow from '@/components/flows/ReportFlow';
import {
  getSymptomResourceRecommendation,
  isTohonoNationZip,
  type ResourceAction,
} from '@/utils/symptomResources';
...
export default function HomeScreen() {
  const { loc } = useLang();
  const toNationButtonLabel = "Tohono O'odham Resources";
  ...
  const [selectedNationResources, setSelectedNationResources] = useState<ResourceAction[]>([]);
  ...
  const load = useCallback(async () => {
    const [storedZip, storedStats, reports] = await Promise.all([
      getUserZip(),
      getReportStats(),
      getMyReports(),
    ]);
  ...
  }, [load, pulse1, pulse2, screenOpacity, tagOpacity, textOpacity, textSlide]);
  ...
  const resourceRecommendation = getSymptomResourceRecommendation(latestReport);
  const showNationResourcesButton = Boolean(
    resourceRecommendation?.nationResources?.length
      && isTohonoNationZip(latestReport?.zip),
  );

  const handleResourceAction = useCallback(async (action: ResourceAction) => {
  ...
  const openNationResources = useCallback((resources: ResourceAction[]) => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    setSelectedNationResources(resources);
    setShowNationResources(true);
  }, []);

  const closeNationResources = useCallback(() => {
    setShowNationResources(false);
  }, []);

  const handleNationResourceAction = useCallback(async (action: ResourceAction) => {
    setShowNationResources(false);
    await handleResourceAction(action);
  }, [handleResourceAction]);

  const getActionDetail = useCallback((action: ResourceAction) => {
    if (action.detail) return action.detail;
    if (action.type === 'call' && action.target.startsWith('tel:')) {
      const digits = action.target.replace('tel:', '');
      if (digits.length === 10) {
  ...
  const getActionLabel = useCallback((action: ResourceAction) => {
    if (action.target === 'tel:911') return loc.res_call_911 || action.label;
    if (action.target.includes('pima.gov/3153')) return loc.res_pima_data || action.label;
    if (action.target.includes('azdhs.gov')) return loc.res_az_dashboard || action.label;
    if (action.target.includes('rocky-mountain-spotted-fever')) return loc.res_cdc_rmsf || action.label;
    if (action.type === 'route') return loc.res_open_map || action.label;
    return action.label;
  }, [loc]);

  const resourceTone = resourceRecommendation?.tone === 'danger'
    ? { bg: '#FFF2F1', border: '#F5C4C1', accent: '#B42318', soft: '#FEE4E2' }
    : resourceRecommendation?.tone === 'warning'
      ? { bg: '#FFF8ED', border: '#F2D3A2', accent: '#B95C00', soft: '#FFE7C2' }
      : { bg: t.card, border: '#E7EFE7', accent: t.accent, soft: t.accentSoft };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.bg }}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}
          ...
          <View style={{ paddingHorizontal: 24, marginTop: 12 }}>
            ...
            {!resourceRecommendation ? (
              <View style={{ backgroundColor: t.card, borderRadius: 16, padding: 18 }}>
                <Text style={{ color: t.text, fontSize: 17, fontFamily: 'Manrope_700Bold', letterSpacing: -0.4 }}>
                  {loc.res_placeholder_title || 'Report symptoms to unlock resources'}
                </Text>
                <Text style={{ color: t.sub, fontSize: 14, lineHeight: 21, fontFamily: 'Manrope_400Regular', marginTop: 8 }}>
                  {loc.res_placeholder_body || 'We will match your latest report to local guidance and surveillance links.'}
                </Text>
                <TouchableOpacity
                  activeOpacity={0.85}
                  onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); router.push('/report-modal'); }}
                  style={{ backgroundColor: t.accent, borderRadius: 14, paddingVertical: 14, alignItems: 'center', marginTop: 16 }}
                >
                  <Text style={{ color: '#FFF', fontSize: 14, fontFamily: 'Manrope_600SemiBold' }}>
                    {loc.res_submit_report || 'Report symptoms'}
                  </Text>
                </TouchableOpacity>
              </View>
            ) : (
              <View style={{
                backgroundColor: resourceTone.bg,
                borderRadius: 16,
                padding: 18,
                borderWidth: 1.5,
                borderColor: resourceTone.border,
              }}>
                <View style={{ flexDirection: 'row', gap: 12, alignItems: 'flex-start' }}>
                  <View style={{
                    ...
                    height: 42,
                    borderRadius: 12,
                    backgroundColor: resourceTone.soft,
                    alignItems: 'center',
                    justifyContent: 'center',
                  }}>
                    <Ionicons name={resourceRecommendation.icon} size={20} color={resourceTone.accent} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: t.text, fontSize: 17, fontFamily: 'Manrope_700Bold', letterSpacing: -0.4 }}>
                      {resourceRecommendation.title}
                    </Text>
                    <Text style={{ color: t.sub, fontSize: 14, lineHeight: 21, fontFamily: 'Manrope_400Regular', marginTop: 6 }}>
                      {resourceRecommendation.summary}
                    </Text>
                  </View>
                </View>

                {resourceRecommendation.matchedSymptoms.length > 0 && (
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 14 }}>
                    {resourceRecommendation.matchedSymptoms.map((symptom) => (
                      <View key={symptom} style={{
                        ...
                        <Text style={{ color: resourceTone.accent, fontSize: 12, fontFamily: 'Manrope_600SemiBold' }}>
                          {symptom}
                        </Text>
                      </View>
                    ))}
                ...
                <Text style={{ color: t.sub, fontSize: 12, lineHeight: 18, fontFamily: 'Manrope_500Medium', marginTop: 14 }}>
                  {resourceRecommendation.reason}
                </Text>
                {latestReport?.date && (
                  <Text style={{ color: t.hint, fontSize: 11, fontFamily: 'Manrope_500Medium', marginTop: 6 }}>
                    {(loc.res_based_on || 'Based on your report from') + ' ' + latestReport.date}
                  </Text>
                )}

                <View style={{ gap: 10, marginTop: 16 }}>
                  {resourceRecommendation.actions.map((action) => {
                    const isPrimary = action.primary !== false;
                    return (
                      <TouchableOpacity
                        key={`${action.type}-${action.target}`}
                        activeOpacity={0.85}
                        onPress={() => { void handleResourceAction(action); }}
                        style={{
                          backgroundColor: isPrimary ? resourceTone.accent : '#FFFFFFB8',
                          borderRadius: 14,
                          paddingVertical: 14,
                      ...
                      >
                        <Text style={{
                          color: isPrimary ? '#FFF' : resourceTone.accent,
                          fontSize: 14,
                  ...
                  {showNationResourcesButton && (
                    <TouchableOpacity
                      activeOpacity={0.85}
                      onPress={() => openNationResources(resourceRecommendation.nationResources ?? [])}
                      style={{
                        backgroundColor: resourceTone.accent,
                        borderRadius: 14,
                        paddingVertical: 14,
                        paddingHorizontal: 14,
                        alignItems: 'center',
        ...
        <Modal
          visible={showNationResources}
          transparent
          animationType="fade"
          onRequestClose={closeNationResources}
        >
          <View style={{
            flex: 1,
            ...
            <TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={closeNationResources} />
            <View style={{
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <Text style={{ color: t.text, fontSize: 20, fontFamily: 'Manrope_700Bold', letterSpacing: -0.5 }}>
                  {toNationButtonLabel}
                </Text>
                <TouchableOpacity
                  onPress={closeNationResources}
                  style={{
                    width: 34,
                    height: 34,
                    borderRadius: 17,
              ...
              <Text style={{ color: t.sub, fontSize: 13, lineHeight: 20, fontFamily: 'Manrope_400Regular', marginBottom: 16 }}>
                {"Tap a resource to call the relevant Tohono O'odham or partner contact."}
              </Text>

              <ScrollView showsVerticalScrollIndicator={false}>
                <View style={{ gap: 10, paddingBottom: 8 }}>
                  {selectedNationResources.map((action) => (
                    <TouchableOpacity
                      key={`${action.label}-${action.target}`}
                      activeOpacity={0.85}
                      onPress={() => { void handleNationResourceAction(action); }}
                      style={{
                    ...
                    >
                      <Text style={{ color: t.text, fontSize: 15, fontFamily: 'Manrope_600SemiBold' }}>
                        {action.label}
                      </Text>
                      <Text style={{ color: t.accent, fontSize: 13, fontFamily: 'Manrope_500Medium', marginTop: 4 }}>
